...
func TestAes(t *testing.T) {
	...
	dst, err := AesECBEncrypt(src, key, PKCS7PADDING)
...
func TestAesECBDecrypt(t *testing.T) {
	...
	// AES-128-ECB, PKCS7PADDING
	key := []byte("1234512345123451")
	dst, err := AesECBDecrypt(src, key, PKCS7PADDING)
	assert.NoError(t, err)
	t.Log(string(dst))
...
func TestAesCBCEncrypt(t *testing.T) {
	...
	// AES-128-ECB, PKCS7PADDING
	key := []byte("1234512345123451")
	dst, err := AesCBCEncrypt(src, key, iv, PKCS7PADDING)
	assert.NoError(t, err)
	t.Log(base64.StdEncoding.EncodeToString(dst))
...
func TestAesCBCDecrypt(t *testing.T) {
	...
	// AES-128-ECB, PKCS7PADDING
	key := []byte("1234512345123451")
	dst, err := AesCBCDecrypt(src, key, iv, PKCS7PADDING)
	assert.NoError(t, err)
	t.Log(string(dst))
	assert.Equal(t, dst, []byte("123456"))
}
