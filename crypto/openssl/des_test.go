...
func TestDesEncrypt(t *testing.T) {
	...
	key := []byte("12345123")
	dst, err := DesECBEncrypt(src, key, PKCS7PADDING)
...
func TestDesECBDecrypt(t *testing.T) {
	...
	// DES-ECB, PKCS7PADDING
	key := []byte("12345123")
	dst, err := DesECBDecrypt(src, key, PKCS7PADDING)
	assert.NoError(t, err)
	t.Log(string(dst))
...
func TestDesCBCEncrypt(t *testing.T) {
	...
	// DES-ECB, PKCS7PADDING
	key := []byte("12345123")
	dst, err := DesCBCEncrypt(src, key, iv, PKCS7PADDING)
	assert.NoError(t, err)
	t.Log(base64.StdEncoding.EncodeToString(dst))
...
func TestDesCBCDecrypt(t *testing.T) {
	...
	// DES-ECB, PKCS7PADDING
	key := []byte("12345123")
	dst, err := DesCBCDecrypt(src, key, iv, PKCS7PADDING)
	assert.NoError(t, err)
	t.Log(string(dst))
	assert.Equal(t, dst, []byte("123456"))
}
